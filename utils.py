...
def get_angles(proj_log_csv, first_angle=0):
    ...
    if df.shape[1] < 3:
        raise ValueError(
            "proj_log.csv should have at least 3 columns; the 3rd column holds angles."
        )
...
def get_projections(corr_dir):
    ...
    if len(tiff_paths) == 0:
        raise FileNotFoundError(f"No TIFF projections found in {corr_dir}")
    ...
    arr = itk.array_from_image(projs).astype(
        np.float32
    )  # shape: (Nproj, Y, X) or (Z,Y,X) depending on ITK build
